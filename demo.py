...
def main(Params, optimize=False):
    if optimize:
        ...
        study.optimize(lambda trial: objective(trial, Params), n_trials=3)
        print('Best trial:')
        ...
        best_weights = trial.params
        Params.update(best_weights)
    ...
    for split in range(0, numRuns):
        ...
        # for signals, labels, idx in test_loader:
        #     print("Batch signals:", signals)
        ...
        if mode == 'teacher':
            ...
            model_ft = Lightning_Wrapper(
                ...
                num_classes=Params['num_classes'][Dataset],  
                max_iter=len(train_loader), label_names=Params['class_names'],
            )
        else:
...
def parse_args():
    ...
    parser.add_argument('--folder', type=str, default='Saved_Models/optuna_t/', help='Location to save models')
    ...
    parser.add_argument('--num_epochs', type=int, default=50, help='Number of epochs to train each model for (default: 50)')
    ...
    parser.add_argument('--patience', type=int, default=100, help='Number of epochs to train each model for (default: 50)')
