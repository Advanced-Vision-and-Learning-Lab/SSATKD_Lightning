...
from Utils.pycontourlet.pycontourlet4d.pycontourlet import batch_multi_channel_pdfbdec
import pdb
...
class ContourletCNN(nn.Module):
    ...
    """
    def __init__(self, in_channels=16, n_levs=[0, 3, 3, 3], variant="SSF", spec_type="all"):
    ...
    def __pdfbdec(self, x, method="resize"):
        ...
        # Obtain coefficients
        coefs = batch_multi_channel_pdfbdec(x=x, pfilt="maxflat", 
                                            dfilt="dmaxflat7", 
                                            nlevs=self.n_levs, device=self.device)
