...
def dfilters(fname, type, device=torch.device("cpu")):
    ...
    if fname == "haar":
        if type.lower() == 'd':
            h0 = torch.tensor([1, 1], dtype=torch.float32, device=device) / sqrt(2)
            h1 = torch.tensor([-1, 1], dtype=torch.float32, device=device) / sqrt(2)
        else:
            h0 = torch.tensor([1, 1], dtype=torch.float32, device=device) / sqrt(2)
            h1 = torch.tensor([1, -1], dtype=torch.float32, device=device) / sqrt(2)
        return h0, h1
        ...
        h0 = torch.tensor(mctrans(h0, t), dtype=torch.float32, device=device)
        h1 = torch.tensor(mctrans(h1, t), dtype=torch.float32, device=device)
        return h0, h1
    elif fname == "ko":
        a0, a1, a2 = 2, 0.5, 1
        ...
        h0 = torch.tensor([[0, -a1, -a0 * a1, 0],
                           [-a2, -a0 * a2, -a0, 1],
                           [0, a0 * a1 * a2, -a1 * a2, 0]], dtype=torch.float32, device=device)

        h1 = torch.tensor([[0, -a1 * a2, -a0 * a1 * a2, 0],
                           [1, a0, -a0 * a2, a2],
                           [0, -a0 * a1, a1, 0]], dtype=torch.float32, device=device)
        ...
        h0 = torch.tensor(h0, dtype=torch.float32, device=device)
        h1 = torch.tensor(h1, dtype=torch.float32, device=device)
        ...
        h0 = sqrt(2) * np.vstack((np.hstack((h, h[:, len(h) - 2::-1])),
                                  np.hstack((h[len(h) - 2::-1, :],
        ...
        h0 = sqrt(2) * np.vstack((np.hstack((h[len(h):0:-1, len(h):0:-1], h[len(h):0:-1, :])),
                                  np.hstack((h[:, len(h):0:-1], h))))
        ...
        for n1 in range(-m, m + 1):
            for n2 in range(-n, n + 1):
                h[n1 + m, n2 + n] = .5 * np.sinc((n1 + n2) / 2.0) * .5 * np.sinc((n1 - n2) / 2.0)
        ...
        h0 = mctrans(h, B)
        g0 = mctrans(g, B)
        h0 = sqrt(2) * h0 / np.sum(h0)
        g0 = sqrt(2) * g0 / np.sum(g0)
        ...
        h = np.array([.25 * k2 * k3, .5 * k2, 1 + .5 * k2 * k3]) * M1
        h = np.hstack((h, h[len(h) - 2::-1]))
        g = np.array([-.125 * k1 * k2 * k3, 0.25 * k1 * k2,
